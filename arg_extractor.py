...
def parse_args():
    ...
    parser.add_argument('--model_name', type=str, choices=AVAILABLE_MODELS, default=AVAILABLE_MODELS[0], help='Name of the model to train')
    ...
    # System
    parser.add_argument('--use_gpu', type=str2bool, help='Whether to use GPU')
...
def merge_configs(yaml_config, cli_args):
    ...
    merged_config = {
        ...
        "WEIGHT_DECAY": cli_args.weight_decay_coefficient if cli_args.weight_decay_coefficient is not None else 0.0,
        "CONTINUE_FROM_EPOCH": cli_args.continue_from_epoch,
        "LOAD_ENCODER_WEIGHTS": cli_args.load_encoder_weights,
        "USE_GPU": cli_args.use_gpu if cli_args.use_gpu is not None else (yaml_config.get("DEVICE") == "cuda"),
